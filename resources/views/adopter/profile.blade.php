...
<table class="table table-striped table-hover">
    ...
    <tr>
      <td>{{$adopter->id}}</td>
      <td>{{$adopter->adopter_Fname}}</td>    
      <td>{{$adopter->adopter_Lname}}</td>
      <td>{{$adopter->adopter_Address}}</td>
      <td>{{$adopter->adopter_Contact}}</td>
      <td>{{$adopter->adopter_Email}}</td>
      <td><img src="{{asset($adopter->img)}}" width="150"> </td>
       @if ($adopter->isApproved==1)
      <td align="center"><a href="{{ route('adopteredit.profile',$adopter->id)  }}"><i class="fa fa-pencil-square-o" aria-hidden="true" style="font-size:24px" ></a></i></td>
      @endif
      </tr>
